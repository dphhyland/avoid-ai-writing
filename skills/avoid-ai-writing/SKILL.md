...
<!-- BEGIN mine (personal) -->
**Voice `mine`:** never Americanise spelling. Australian/British forms (`-ise`/`-isation`, `-our`, `-re`) are correct, not flags — treat a US swap as an error, not a fix.
<!-- END mine (personal) -->
...
<!-- BEGIN mine (personal) -->
**`mine`** (David Hyland) — One voice, two registers; which applies follows the context profile (`casual`, and email to a known contact → **casual register**; `blog` / `docs` / `technical-blog` / `linkedin` long-form → **considered register**). Shared spine, both registers: **Australian/British spelling and idiom** — `-ise`/`-isation`, `-our`, `-re`; never Americanise (treat a US swap as an error, not a fix); AU idiom stays ("Mums and Dads", "have a good weekend"). The **spaced dash** (` - ` casual, ` – ` considered) is his connector/appositive — keep it, never rewrite it to a comma or period, and never count it toward the em-dash rate. One **vivid image or first-person call** is allowed to land when it carries a real claim ("information feeds the monster that is the market"; "I believe the shock wont be as big as El-Erian predicts") — flag only decoration with no claim under it, and never flatten his stance into neutrality. No AI scaffolding in either register. **Considered register:** fix the dated exam habits — break 40+-word clause-stacking, `so as to` → `to`, cut copula inflation (`serves as` → `is`) and structural meta-narration ("This paper will describe… It will then…"); keep semicolon enumeration and his `; however,` / `; but,` joins; sentence-case headings. **Casual register:** short; greeting "Hi/Hey [name]", sign-off `dh` / `dave` (lowercase) or "Thanks" / "Apols"; fragments are correct; **preserve the roughness** — dropped apostrophes (`thats`, `Im`, `cant`), fast contractions, abbreviations (`Apols`, `opptys`, `rego`) — fix only a genuinely confusing typo, and only if asked. *Personal profile; auto-detect the register from context.*
<!-- END mine (personal) -->
